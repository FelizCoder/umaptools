...
def setup_parser():
    ...
    my_parser.add_argument(
        ...
        action="store",
        type=int,
        help="number of the umap you want to process",
    )
    my_parser.add_argument(
        "-x",
        action="store",
    ...
    my_parser.add_argument(
        ...
        default="2",
        type=int,
        help="weight (width) of the grid lines",
    )
    my_parser.add_argument(
        "-o",
        "--opacity",
        ...
        default="0.3",
        type=float,
        help="opacity of the grid lines",
    )
...
basicinfo = (
    "'{0}' (umap {1}) {2}\nhttps://umap.openstreetmap.fr/de/map/map_{1}\n\n{3}".format(
        umapname.strip(), umapnumber, now, umapdescription.strip()
    )
)
...
umapcirclefile = "umap-original-ball-icons-changed-to-circle-icons-{0}-{1}.umap".format(
    umapname, umapnumber
)
...
def upolygon(
    coordinates, name=None, description=None, color="Black", weight=4, opacity=0.7
):
    return {
        "type": "Feature",
        "properties": {
    ...
    return {
        ...
        "properties": {
            "_umap_options": {
                ...
                "iconClass": "Drop",
                "iconUrl": "https://raw.githubusercontent.com/Wikinaut/umaptools/main/img/80/circle-{0}.png".format(
                    number
                ),
            },
            "name": name,
...
for i in data["layers"]:
    ...
    if i["_umap_options"]["name"] == "Grid" or i["_umap_options"].get(
        "gridparseIgnore", False
    ):
        continue
